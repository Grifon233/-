...
"""
    ...
    def _apply_resume(
        ...
        site_emails = {}
        applied_file = r"C:\Users\ЗС\OneDrive\Рабочий стол\Атокликер хх ру\applied_companies.txt"
        try:
            with open(applied_file, "r", encoding="utf-8") as f:
                applied_companies = set(f.read().splitlines())
        ...
        for vacancy in self._get_vacancies(resume_id=resume["id"]):
            ...
            try:
                ...
                if employer_id and str(employer_id) in applied_companies:
                    print(f"🔄 Пропуск: уже откликались в компанию {employer.get('name', 'Unknown')}")
                    continue
                    ...
                    try:
                        if not self.dry_run:
                            ...
                            if result.get("success") == "true":
                                ...
                                if employer_id:
                                    applied_companies.add(str(employer_id))
                                    with open(applied_file, "a", encoding="utf-8") as f:
                                        f.write(str(employer_id) + "\n")
                            else:
    ...
    def _is_excluded(self, vacancy: SearchVacancy) -> bool:
        ...
        if not match:
            logger.warning(f"Не удалось получить описание для вакансии {vacancy['id']}")
            return False
