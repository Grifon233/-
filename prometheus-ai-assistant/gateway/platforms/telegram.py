...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Прометей-created private-chat topic lanes are marked with
    ...
    async def send(
        ...
        try:
            ...
            # Re-trigger typing only for intermediate/progress messages.
            # Final replies are sent with metadata["notify"]=True by the base
            # adapter. Re-starting Telegram's typing indicator after the final
            # answer makes the chat still show "typing" for a few seconds and
            # can leave a large visual gap below the last message on mobile.
        ...
        try:
            ...
            keyboard = InlineKeyboardMarkup([
                [
                    ...
                    InlineKeyboardButton("✅ На сессию", callback_data=f"ea:session:{approval_id}"),
                ],
                [
                    InlineKeyboardButton("✅ Всегда", callback_data=f"ea:always:{approval_id}"),
                    InlineKeyboardButton("❌ Запретить", callback_data=f"ea:deny:{approval_id}"),
                ],
            ])
