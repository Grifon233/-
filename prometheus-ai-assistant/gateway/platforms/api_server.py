...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Прометей-Session-Id header; opt-in long-term memory scoping via X-Прометей-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Прометей-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists hermes-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Прометей sessions
- POST /api/sessions               — create an empty Прометей session
...
def _derive_chat_session_id(
    ...
    the same Прометей session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Прометей-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Прометей-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Прометей-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Прометей-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Прометей-Session-Key requires API key authentication. "
    ...
    def _create_agent(
        ...
        by the client (via ``X-Прометей-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Прометей version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "features": {
                ...
                "session_continuity_header": "X-Прометей-Session-Id",
                "session_key_header": "X-Прометей-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Прометей sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Прометей session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Прометей-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Прометей-Session-Key"] = gateway_session_key
        return web.json_response(
            {
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Прометей-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Прометей-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Прометей-Session-Key.  This
        # is independent of X-Прометей-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Прометей-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Прометей-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Прометей-Session-Id rejected: "
            ...
            # conversation map to the same Прометей session.  The first user
        ...
        response_headers = {
            "X-Прометей-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Прометей-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Прометей-Completed"] = "false"
            response_headers["X-Прометей-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Прометей-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Прометей-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Прометей-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Прометей-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Прометей-Session-Key"] = gateway_session_key
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Прометей-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Прометей-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Прометей-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Прометей-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Прометей-Relay
