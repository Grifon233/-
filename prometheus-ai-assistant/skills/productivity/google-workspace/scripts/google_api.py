#!/usr/bin/env python3
"""Google Workspace API CLI for Прометей.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Прометей-facing JSON contract and falls back to the Python client
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Прометей")
