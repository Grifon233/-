...
def summarize_background_review_actions(
    ...
    for msg in review_messages or []:
        ...
        if "created" in message.lower():
            if russian:
                match = re.match(r"Skill '(.+)' created\.?", message, re.IGNORECASE)
                actions.append(
                    f"Навык «{match.group(1)}» создан" if match
                    else "Создан новый навык"
                )
            else:
                actions.append(message)
        elif "updated" in message.lower():
            if russian:
                match = re.match(r"Skill '(.+)' updated\.?", message, re.IGNORECASE)
                actions.append(
                    f"Навык «{match.group(1)}» обновлён" if match
            ...
            if russian:
                label = (
                    "Долговременная память" if target == "memory"
                    else "Профиль пользователя" if target == "user"
                    else target
                )
                actions.append(f"{label} обновлена")
            else:
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require Прометей's own dispatch,
