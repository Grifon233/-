...
def busy_input_hint_gateway(mode: str) -> str:
    ...
    if mode == "queue":
        return (
            "💡 Подсказка на будущее: я принял сообщение и дождусь завершения текущей задачи. "
            "Отправьте `/busy interrupt`, чтобы новые сообщения сразу останавливали текущую задачу, "
            "или `/busy status`, чтобы проверить режим. Эта подсказка больше не появится."
        )
    if mode == "steer":
        return (
            "💡 Подсказка на будущее: я добавил ваше сообщение к текущей задаче; "
            "оно будет учтено на ближайшем шаге, не прерывая работу. "
            "Отправьте `/busy interrupt` или `/busy queue`, чтобы изменить режим, "
            "или `/busy status`, чтобы проверить его. Эта подсказка больше не появится."
        )
    return (
        "💡 Подсказка на будущее: я прервал текущую задачу, чтобы ответить вам. "
        "Отправьте `/busy queue`, чтобы следующие сообщения ждали завершения текущей задачи, "
        "`/busy steer`, чтобы добавлять их к текущей задаче без прерывания, "
        "или `/busy status`, чтобы проверить режим. Эта подсказка больше не появится."
    )
...
def tool_progress_hint_gateway() -> str:
    return (
        "💡 Подсказка на будущее: задача заняла больше времени, поэтому я показываю ход работы. "
        "Если таких сообщений слишком много, отправьте `/verbose`, чтобы переключить режим "
        "(all → new → off). Эта подсказка больше не появится."
    )
