...
Owns one Codex thread per Прометей session. Drives `turn/start`, consumes
...
def _classify_oauth_failure(*parts: str) -> Optional[str]:
    ...
    for needle in _OAUTH_REFRESH_FAILURE_HINTS:
        if needle in haystack:
            return (
                "Не удалось авторизоваться в Codex — вход через ChatGPT/Codex "
                "похоже истёк или стал недействительным. Выполните `codex login`, "
                "затем повторите запрос. Если проблема останется, временно "
                "вернитесь к обычному runtime командой `/codex-runtime auto`."
            )
    return None
...
class CodexAppServerSession:
    """One Codex thread per Прометей session, lifetime owned by AIAgent.
...
def _get_hermes_version() -> str:
    """Best-effort Прометей version string for codex's userAgent line."""
    try:
