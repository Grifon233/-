"""Anthropic Messages API adapter for Прометей.

Translates between Прометей's internal OpenAI-style message format and
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    # 1. Прометей-managed OAuth/setup token env var
...
# ── Прометей-native PKCE OAuth flow ────────────────────────────────────────
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Прометей-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Прометей-managed OAuth credentials from ~/.hermes/.anthropic_oauth.json."""
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Прометей", "Claude Code")
    ...
    # silently hides reasoning text that Прометей surfaces in its CLI. We
