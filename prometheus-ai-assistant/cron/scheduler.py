...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    # Optionally wrap the content with a lightweight user-facing header.
    # Clean output is the default: reminders and scheduled messages should
    # read like normal bot messages, not internal scheduler reports.
    wrap_response = False
    try:
    ...
    if wrap_response:
        ...
        delivery_content = (
            f"🔔 {task_name}\n\n"
            f"{content}\n\n"
            "Если нужно изменить или остановить это напоминание, просто напишите мне об этом."
        )
    else:
