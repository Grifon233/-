#!/usr/bin/env python3
"""
SQLite State Store for Прометей.
...
class SessionDB:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Прометей surface and jump
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Прометей session.

        A Прометей session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Прометей session is already bound to any Telegram DM topic.
