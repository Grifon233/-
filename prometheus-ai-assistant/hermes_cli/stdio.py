...
on Japanese locales, etc.).  Прометей's banners, tool output feed, and slash
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Прометей-managed Git install directories so
    ...
    # install without Прометей-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Прометей's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Прометей's
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Прометей-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Patch-up strategy: add the known Прометей-managed tool directories to our
