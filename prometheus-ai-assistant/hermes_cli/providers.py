"""
Single source of truth for provider identity in Прометей.
...
# -- Hermes overlay ----------------------------------------------------------
# Прометей-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Прометей-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    if overlay is not None:
        # Прометей-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
