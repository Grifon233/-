...
app = FastAPI(title="Прометей", version=__version__, lifespan=_lifespan)
...
_SESSION_HEADER_NAME = "X-Прометей-Session-Token"
...
async def check_hermes_update(force: bool = False):
    """Report whether a Прометей update is available, without applying it.
    ...
    Returns:
        ...
        current_version: installed Прометей version string
...
async def search_sessions(q: str = "", limit: int = 20):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Прометей surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Прометей supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Прометей from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Прометей to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Прометей from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Прометей to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Прометей in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Прометей through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Прометей via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Прометей through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Прометей to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Прометей inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Прометей through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Прометей to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Прометей to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Прометей as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "https://hermes-agent.nousresearch.com/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hermes/.anthropic_oauth.json`` — Прометей-managed PKCE flow
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    when they also have a separate Прометей-managed PKCE login.
    """
    try:
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Прометей-managed PKCE file
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with Прометей exposes a
