...
What does NOT translate (warned + skipped):
  Прометей-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Прометей's own
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Прометей's own
...
def migrate(
    ...
    *,
    ...
    Args:
        ...
        expose_hermes_tools: when True (default), register Прометей's own
    ...
    # Inject Прометей's own tool surface as an MCP server so the spawned
