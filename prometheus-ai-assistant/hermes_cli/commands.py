"""Slash command definitions and autocomplete for the Прометей CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("update", "Update Прометей to the latest version", "Info"),
    ...
    # --- Multi-user allowlist (gateway-only) ---
    # Owner can add/remove authorized Telegram users. State is persisted
    # in HERMES_HOME/multi_user.json (chmod 600).
    CommandDef("adduser", "Authorize another Telegram user to use this bot",
               "Configuration", gateway_only=True, args_hint="<user_id>"),
    CommandDef("removeuser", "Revoke a Telegram user's access to this bot",
               "Configuration", gateway_only=True, args_hint="<user_id>"),
    CommandDef("users", "List the Telegram users authorized to use this bot",
               "Info", gateway_only=True),
    CommandDef("transferowner", "Hand owner status to another authorized user",
               "Configuration", gateway_only=True, args_hint="<user_id>"),
]
