"""
Multi-provider authentication system for Прометей.
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Прометей's own Codex token store.
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Прометей's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Прометей's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.hermes/auth.json) called POST /api/oauth/token with Прометей's "
...
def _login_openai_codex(
    ...
    *,
    ...
    # Check for existing Прометей-owned credentials
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=hermes-agent` lets xAI attribute Прометей-originated logins
