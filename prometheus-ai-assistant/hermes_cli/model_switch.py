...
_HERMES_MODEL_WARNING = (
    "Прометей Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with Прометей. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Прометей Hermes 3 / Hermes 4 chat families.
...
#   NousResearch/Прометей-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
...
def list_authenticated_providers(
    ...
    # --- 1. Check Прометей-mapped providers ---
    from hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # --- 2. Check Прометей-only providers (nous, openai-codex, copilot, opencode-go) ---
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Прометей's canonical write key (matches
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Прометей's own writer uses; a hyphen variant is accepted
            ...
            # active model. Прометей's own writer (main.py::_save_custom_provider)
