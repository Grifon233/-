...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Прометей's own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Прометей-managed storage. A board ``default_workdir`` pointing at a
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Прометей CLI invocation."""
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Прометей or activate its venv before running the kanban dispatcher."
        )
