"""
Configuration management for Прометей.
...
Прометей runs as a published image (nousresearch/hermes-agent), not a
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Прометей-level retry loop that wraps the whole call.  Lower
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "✦ *Прометей*" header.
    ...
    "cron": {
        # Deliver clean scheduled-message output by default.  Set true to add
        # a lightweight user-facing reminder header.
        "wrap_response": False,
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Прометей versions) write ``models`` as
