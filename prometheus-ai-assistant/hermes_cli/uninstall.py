    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Прометей" comment and following line
                if '# Прометей' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Прометей-owned User-PATH entries."""
...
def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Прометей-owned entries from User-scope PATH in the registry.
...
def run_uninstall(args):
    ...
    print(color("│            ⚕ Прометей Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Прометей-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Прометей-set User env vars to remove")
    ...
    log_info("Removing Прометей-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Прометей-managed node/npm/npx symlinks found")
    ...
    print()
    print("Thank you for using Прометей! ⚕")
    print()
