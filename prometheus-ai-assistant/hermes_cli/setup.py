"""
Interactive setup wizard for Прометей.
...
def _print_migration_preview(report: dict):
    ...
    if warnings_shown:
        ...
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Прометей's yolo mode.", Colors.YELLOW))
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Прометей session — model selected, provider set, and web/image/tts/browser
...
def run_setup_wizard(args):
    ...
    print(
        color(
            "│             ⚕ Прометей Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Прометей installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
