...
Usage:
    ...
    hermes uninstall           Uninstall Прометей
...
def _print_fast_version_info() -> None:
    ...
    print(f"Прометей v{__version__} ({__release_date__})")
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Прометей'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_login(args):
    """Authenticate Прометей CLI with a provider."""
...
def _update_via_zip(args):
    """Update Прометей by downloading a ZIP archive.
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Прометей — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def cmd_update(args):
    """Update Прометей to the latest version.
...
def cmd_dashboard(args):
    ...
    # the same logs directory as every other Прометей surface.
    try:
        from hermes_logging import setup_logging as _setup_logging_gui
        _setup_logging_gui(mode="gui")
    except Exception:
...
def main():
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Прометей with an interactive wizard. "
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Прометей CLI",
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Прометей components",
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Прометей setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Прометей. Use 'hermes debug share' to "
...
""",
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Прометей configuration",
    )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Прометей to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Прометей",
        description="Remove Прометей from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Прометей as an ACP (Agent Client Protocol) server",
        description="Start Прометей in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    def cmd_acp(args):
        """Launch Прометей as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Прометей web dashboard for managing config, API keys, and sessions",
    )
    ...
    # for one release. The Прометей-Setup.exe success screen tells users to
