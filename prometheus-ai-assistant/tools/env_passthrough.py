...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a Прометей-managed provider credential (API key,
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Прометей-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Прометей-managed
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Прометей-managed provider credentials must not be passed
