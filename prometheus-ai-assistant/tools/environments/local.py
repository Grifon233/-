...
# Прометей-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# Прометей-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Прометей-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    raise RuntimeError(
        "Git Bash not found. Прометей requires Git for Windows on Windows.\n"
