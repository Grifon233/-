...
# Pre-install preview templates — short, multi-line, chat-friendly.
# Used by do_install() to show the user what was scanned before the
# "continue? y/n" prompt, so a non-technical user gets a concrete picture
# of what they're about to install instead of just "verdict: caution".
_VERDICT_EMOJI = {
    "safe":       "✅",
    "caution":    "⚠️",
    "dangerous":  "⛔",
...
def build_install_preview(result: ScanResult, *, max_findings: int = 8) -> str:
    """Build a compact, chat-friendly preview of a scanned skill.

    The output is the thing a non-technical user sees right before
    "Install? (y/n)". It's intentionally shorter than
    :func:`format_scan_report`: at most ``max_findings`` findings are
    listed, grouped by category, and severity is summarized in one line
    so a safe scan doesn't take up screen real estate.

    Example output (caution verdict)::

        ⚠️ Skill: my-cool-skill (community)
        Files: 4 · Findings: 3 (2 medium, 1 low)

        Findings shown (3):
          🟡 [exfiltration] SKILL.md:12  curl with $KEY
          🟡 [obfuscation] scripts/run.sh:5  base64 decode + exec
          🔵 [injection]  SKILL.md:45   "ignore previous instructions"
        (5 more — full report: /skills audit my-cool-skill)

        Community source. Install anyway? (y/n)

    Safe verdicts collapse the findings block to a single line so
    power-users aren't slowed down.
    """
    if not isinstance(result, ScanResult):  # type: ignore[unreachable]
        return "(no scan result)"

    emoji = _VERDICT_EMOJI.get(result.verdict, "❓")
    lines = [
        f"{emoji} {result.skill_name} ({result.source}/{result.trust_level})",
    ]

    # Severity breakdown (count per severity).
    ...
    lines.append(f"Findings: {sev_summary}")

    if not result.findings:
        return "\n".join(lines)

    # Group by category, sort by severity within each group, dedupe.
    severity_order = {"critical": 0, "high": 1, "medium": 2, "low": 3}
    sorted_findings = sorted(
        result.findings,
        key=lambda f: (severity_order.get(f.severity, 4), f.category, f.file, f.line),
    )
    shown = sorted_findings[:max_findings]
    hidden = len(sorted_findings) - len(shown)

    lines.append("")
    lines.append(f"Findings shown ({len(shown)}):")
    for f in shown:
        sev_emoji = _SEVERITY_EMOJI.get(f.severity, "•")
        match_preview = f.match.strip()
        if len(match_preview) > 80:
            match_preview = match_preview[:77] + "..."
        lines.append(
            f"  {sev_emoji} [{f.category}] {f.file}:{f.line}  {match_preview}"
        )
    if hidden > 0:
...
# scan. `.skillignore` is the Прометей-native name; `.clawhubignore` is honored
