"""
Cron job management tools for Прометей.
...
_REMINDER_ONE_SHOT_RE = re.compile(
    r"\b(?:напомни|напомните|remind\s+me)\b",
    re.IGNORECASE,
)

_REMINDER_RECURRING_RE = re.compile(
    r"\b(?:напоминай|напоминайте|напоминать|кажд(?:ый|ые|ую|ое|ого|ом|ыми)|"
    r"каждые|ежедневн\w*|еженедельн\w*|ежемесячн\w*|регулярн\w*|"
    r"повторяй|повторять|раз\s+в|по\s+будням|по\s+выходным|"
    r"daily|weekly|monthly|every|repeat(?:ing)?)\b",
    re.IGNORECASE,
)


def _parse_ru_count(raw: str) -> Optional[int]:
    text = (raw or "").strip().lower()
    if not text:
        return None
    if text.isdigit():
        return int(text)
    return _RU_NUMBER_WORDS.get(text)


def _schedule_period_days(parsed_schedule: Dict[str, Any]) -> Optional[float]:
    kind = parsed_schedule.get("kind")
    if kind == "interval":
        minutes = parsed_schedule.get("minutes")
        if isinstance(minutes, (int, float)) and minutes > 0:
            return float(minutes) / 1440
        return None

    if kind == "cron":
        expr = str(parsed_schedule.get("expr") or "")
        parts = expr.split()
        if len(parts) != 5:
            return None
...
def _repeat_from_reminder_duration(text: str, parsed_schedule: Dict[str, Any]) -> Optional[int]:
    lowered = text.lower()

    explicit_times = re.search(
        r"\b(\d+|один|одна|одно|два|две|три|четыре|пять|шесть|семь|восемь|девять|десять)\s+раз(?:а|ок)?\b",
        lowered,
        re.IGNORECASE,
    )
    if explicit_times:
        return _parse_ru_count(explicit_times.group(1))

    days: Optional[float] = None
    duration = re.search(
        r"\b(?:в\s+течение|на)\s+(\d+|одну|один|одна|два|две|три|четыре|пять|шесть|семь|восемь|девять|десять)?\s*"
        r"(дн(?:я|ей|ь)?|недел(?:ю|и|ь)?|месяц(?:а|ев)?)\b",
        lowered,
        re.IGNORECASE,
    )
    if duration:
        count = _parse_ru_count(duration.group(1) or "один") or 1
    ...
    if days is None:
        if re.search(r"\b(?:в\s+течение|на)\s+недел[юи]\b", lowered, re.IGNORECASE):
            days = 7.0
        elif re.search(r"\b(?:в\s+течение|на)\s+месяц\b", lowered, re.IGNORECASE):
            days = 30.0

    if days is None:
        return None

    period_days = _schedule_period_days(parsed_schedule)
    if not period_days:
        return None
    return max(1, int(math.ceil(days / period_days)))


def _infer_reminder_repeat(
    *,
    prompt: Optional[str],
    name: Optional[str],
    schedule: str,
    ...
    """Infer repeat limits for natural-language reminder requests.

    The LLM usually chooses the right schedule, but Telegram phrasing like
    "напомни мне завтра" can be accidentally converted to a recurring
    schedule. This guard keeps one-shot Russian reminder intent from becoming
    a forever cron job, while still allowing explicit "напоминай каждый день"
    requests to remain recurring.
    """
    ...
    text = " ".join(part for part in (prompt, name) if part).strip()
    if not text:
        return repeat

    finite_repeat = _repeat_from_reminder_duration(text, parsed_schedule)
    if finite_repeat is not None:
        return finite_repeat

    has_one_shot_cue = bool(_REMINDER_ONE_SHOT_RE.search(text))
    has_recurring_cue = bool(_REMINDER_RECURRING_RE.search(text))
    if has_one_shot_cue and not has_recurring_cue:
        return 1
    return repeat
...
Reminder semantics for Russian Telegram requests:
- "напомни мне ..." without recurring words means a one-time reminder. Prefer a one-shot schedule
  (duration like "1d"/"7d" or an ISO timestamp). If a recurring schedule is accidentally used, set repeat=1.
- "напоминай мне ...", "каждый день", "раз в неделю", "ежедневно" means recurring.
- If the user gives a finite window/count ("каждый день в течение недели", "3 раза"),
  set repeat to the number of expected reminder sends.
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "schedule": {
                "type": "string",
                "description": "REQUIRED for action=create. For create/update: '30m', 'every 2h', '0 9 * * *', or ISO timestamp. Examples: '30m' (one-shot in 30 minutes), '7d' (one-shot in 7 days), 'every 2h' (recurring every 2 hours), '0 9 * * *' (recurring daily at 9am), '2026-06-01T09:00:00' (one-shot). For Russian 'напомни мне завтра/через неделю' use a one-shot schedule, not 'every ...'. You MUST include this field when action=create."
            },
            "name": {
                "type": "string",
            ...
            "repeat": {
                "type": "integer",
                "description": "Optional repeat count. Omit for defaults (once for one-shot, forever for recurring). For finite reminders set it explicitly: 'каждый день в течение недели' => repeat=7, '3 раза' => repeat=3. For plain 'напомни мне ...' use repeat=1 if the schedule is recurring."
            },
            "deliver": {
                "type": "string",
