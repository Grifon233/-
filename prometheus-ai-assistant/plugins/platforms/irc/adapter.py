"""
IRC Platform Adapter for Прометей.
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Прометей")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Прометей shutting down")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Прометей (cron)")
