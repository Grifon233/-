#!/usr/bin/env python3
"""
Прометей CLI - Interactive Terminal Interface

A beautiful command-line interface for the Прометей, inspired by Claude Code.
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Прометей") if _skin else "Прометей"
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Прометей v{_version} ({_release_date})"
    else:
...
class HermesCLI:
    """
    Interactive CLI for the Прометей.
    ...
    def show_banner(self):
        ...
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Прометей Hermes 3 & 4 models are NOT agentic and are not "
                "designed for use with Прометей.[/]"
            )
            self._console_print(
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Прометей-managed isolated debug "
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Прометей to the latest version.
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Прометей')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Прометей"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Прометей",
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Привет, я Прометей, готов работать.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Привет, я Прометей, готов работать."
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hermes_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Прометей")
...
def main(
    ...
    """
    Прометей CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting Прометей Gateway (messaging platforms)...")
